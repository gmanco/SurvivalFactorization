...
public class Randoms extends java.util.Random {
	...
	private static final long serialVersionUID = 8864219956833361269L;

	public static void main(String[] args) {
		// Prints the nextGamma() and oldNextGamma() distributions to
		// System.out for testing/comparison.
		final Randoms r = new Randoms();
		final int resolution = 60;
		final int[] histogram1 = new int[resolution];
		final int[] histogram2 = new int[resolution];
		final int scale = 10;

		for (int i = 0; i < 10000; i++) {
			final double x = 4;
			final int index1 = (int) (r.nextGamma(x) / scale * resolution)
					% resolution;
			final int index2 = (int) (r.oldNextGamma((int) x) / scale * resolution)
					% resolution;
			histogram1[index1]++;
			histogram2[index2]++;
		}
	...
	public synchronized double nextBeta(double alpha, double beta) {
		...
		if (alpha == 1 && beta == 1)
			...
			final double A = alpha - 1, B = beta - 1, C = A + B, L = C
					* Math.log(C), mu = A / C, sigma = 0.5 / Math.sqrt(C);
	...
	public synchronized int nextDiscrete(double[] a, double sum) {
		double b = 0;
		final double r = nextUniform() * sum;
		for (int i = 0; i < a.length; i++) {
			b += a[i];
			if (b > r)
				return i;
	...
	public synchronized double nextGaussian() {
		if (!haveNextGaussian) {
			final double v1 = nextUniform(), v2 = nextUniform();
			double x1, x2;
