...
public class Weka_Utils {
	...
	public static/* @pure@ */String backQuoteChars(String string) {
		...
		final char charsFind[] = { '\\', '\'', '\t', '\n', '\r', '"', '%' };
		final String charsReplace[] = { "\\\\", "\\'", "\\t", "\\n", "\\r",
				"\\\"", "\\%" };
	...
	protected static File createRelativePath(File absolute) throws Exception {
		...
		final String userPath = userDir.getAbsolutePath() + File.separator;
		final String targetPath = new File(absolute.getParent()).getPath()
				+ File.separator;
		final String fileName = absolute.getName();
		...
		final int subdir = targetPath.indexOf(userPath);
		if (subdir == 0) {
			if (userPath.length() == targetPath.length())
				relativePath.append(fileName);
			else {
				final int ll = userPath.length();
			...
			while (temp.indexOf(File.separator) != -1) {
				final int ind = temp.indexOf(File.separator);
				sepCount++;
			...
			while (targetTemp.indexOf(File.separator) != -1) {
				final int ind = targetTemp.indexOf(File.separator);
				final int ind2 = userTemp.indexOf(File.separator);
				final String tpart = targetTemp.substring(0, ind + 1);
				final String upart = userTemp.substring(0, ind2 + 1);
			...
			if (tcount == -1)
				...
				throw new Exception(
						"Can't construct a path to file relative to user "
								+ "dir.");
	...
	public static/* @pure@ */String doubleToString(double value, int width,
		...
		final String tempString = doubleToString(value, afterDecimalPoint);
		char[] result;
		int dotPosition;
	...
	public static boolean getFlag(String flag, String[] options)
		...
		final int pos = getOptionPos(flag, options);
	...
	public static/* @non_null@ */String getOption(String flag, String[] options)
		...
		String newString;
		final int i = getOptionPos(flag, options);
	...
	public static String joinOptions(String[] optionArray) {
		...
		String optionString = "";
		for (final String element : optionArray) {
			if (element.equals(""))
				continue;
	...
	public static double logSumOfExponentials(double[] exponentialTerms) {
		...
		for (final double d : exponentialTerms)
			if (d > maxTerm)
				maxTerm = d;
	...
	public static String[] partitionOptions(String[] options) {
		...
		for (int i = 0; i < options.length; i++)
			if (options[i].equals("--")) {
				options[i++] = "";
				final String[] result = new String[options.length - i];
	...
	public static int probRound(double value, Random rand) {
		...
		if (value >= 0) {
			final double lower = Math.floor(value);
			final double prob = value - lower;
			if (rand.nextDouble() < prob)
				return (int) lower + 1;
			else
				return (int) lower;
		} else {
			final double lower = Math.floor(Math.abs(value));
			final double prob = Math.abs(value) - lower;
			if (rand.nextDouble() < prob)
				return -((int) lower + 1);
			else
				return -(int) lower;
	...
	public static/* @pure@ */double probToLogOdds(double prob) {
		...
		final double p = SMALL + (1.0 - 2 * SMALL) * prob;
	...
	public static/* @pure@ */int round(double value) {
		...
		final int roundedValue = value > 0 ? (int) (value + 0.5) : -(int) (Math
				.abs(value) + 0.5);
	...
	public static/* @pure@ */double roundDouble(double value,
		...
		final double mask = Math.pow(10.0, afterDecimalPoint);
	...
	public static/* @pure@ */int[] stableSort(double[] array) {
		...
		while (i < index.length) {
			numEqual = 1;
			for (int j = i + 1; j < index.length
					&& Weka_Utils.eq(array[index[i]], array[index[j]]); j++)
				numEqual++;
			if (numEqual > 1) {
				helpIndex = new int[numEqual];
				for (int j = 0; j < numEqual; j++)
	...
	public static String unbackQuoteChars(String string) {
		...
		// replace each of the following characters with the backquoted version
		final String charsFind[] = { "\\\\", "\\'", "\\t", "\\n", "\\r",
				"\\\"", "\\%" };
		final char charsReplace[] = { '\\', '\'', '\t', '\n', '\r', '"', '%' };
		final int pos[] = new int[charsFind.length];
		int curPos;
	...
	public static/* @pure@ */double variance(double[] vector) {
		...
		final double result = (sumSquared - sum * sum / vector.length)
				/ (vector.length - 1);
