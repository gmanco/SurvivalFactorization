...
public class SurvivalFactorizationEM_LearnerOPT_Rayleigh {
	...
	private static boolean sumZero(double[][] m) {
		double sum = 0;
	...
	public SurvivalFactorizationEM_LearnerOPT_Rayleigh() {
	...
	private void assign(String assignmentFile, int nCascades, int nFactors,
			double[][] gamma) throws Exception {

		if (assignmentFile != null) {
			final PrintWriter pw = new PrintWriter(new FileWriter(
					assignmentFile));
			final PrintWriter pwFull = new PrintWriter(new FileWriter(
					assignmentFile + ".full"));
	...
	private double computeLogLikelihood(CascadeData cascadeData,
			SurvivalFactorizationEM_Model model,
			SurvivalFactorizationEM_ModelCounters_Rayleigh counters,
			double gamma[][]) {
	...
	private double[] computeLogLikelihoodEvents(CascadeData cascadeData,
		...
		loop: for (int k = 0; k < model.nFactors; k++) {
			...
			double ackDiff = cascadeData.t_max * cascadeData.t_max
					* counters.A_c_k[k] + counters.hat_A_c_k[k] - 2
					* cascadeData.t_max * counters.tilde_A_c_k[k];
			...
			for (final CascadeEvent currentEvent : cascadeData
				...
				if (prevEvent != null) {
					...
					double acukDiff = currentEvent.timestamp
							...
							* counters.A_c_u_k[currentEvent.node][k]
							+ counters.hat_A_c_u_k[currentEvent.node][k] - 2
							* currentEvent.timestamp
							* counters.tilde_A_c_u_k[currentEvent.node][k];
					...
					logLikelihoodEvents[k] += Math.log(currentEvent.timestamp
							* counters.A_c_u_k[currentEvent.node][k]
									- counters.tilde_A_c_u_k[currentEvent.node][k])
									- counters.S_c_u_k[currentEvent.node][k] * acukDiff;
				}
	...
	private SurvivalFactorizationEM_Model iterateEM(CascadeData cascadeData,
		...
		final SurvivalFactorizationEM_ModelCounters_Rayleigh counters = new SurvivalFactorizationEM_ModelCounters_Rayleigh(
				nVertices, nFactors);
		...
		// XXX
		// assign(assignmentFile + "_it_" + 0, nCascades, nFactors, gamma);
		...
		do {
			...
			if (saveIteration == save_step) {
				...
				if (prevlogLikelihood > logLikelihood) {
					...
					System.out
					.format("######\tIteration: %d\tLogLikelihood\t%.5f (Loss: %.4f)\r\n",
					...
					System.out
					.format("Iteration: %d\tLogLikelihood\t%.5f (Improvement: %.4f)\r\n",
							iterationsDone, logLikelihood, improvement);
				}
				...
				// XXX
				// assign(assignmentFile + "_it_" + iterationsDone, nCascades,
				// nFactors, gamma);
			}
		...
		assign(assignmentFile, nCascades, nFactors, gamma);
		...
		for (int c = 0; c < cascadeData.getNCascades(); c++) {
				...
				for (int k = 0; k < model.nFactors; k++) {
					if (prevEvent != null) {
						...
						double acukDiff = currentEvent.timestamp
								* currentEvent.timestamp
								* counters.A_c_u_k[currentEvent.node][k]
										+ counters.hat_A_c_u_k[currentEvent.node][k]
								- 2 * currentEvent.timestamp
								* counters.tilde_A_c_u_k[currentEvent.node][k];
					...
					double prod = gamma[c][k]
							* (counters.tilde_R_c_u_k[currentEvent.node][k] - counters.hat_R_c_u_k[currentEvent.node][k]);
					...
					A_new_den[currentEvent.node][k] += gamma[c][k]
							* (counters.hat_S_c_k[k]
									- counters.hat_S_c_u_k[currentEvent.node][k]
											+ currentEvent.timestamp
											* currentEvent.timestamp
											* (counters.S_c_k[k] - counters.S_c_u_k[currentEvent.node][k])
											- 2
											* currentEvent.timestamp
											* (counters.tilde_S_c_k[k] - counters.tilde_S_c_u_k[currentEvent.node][k]) + (cascadeData.t_max - currentEvent.timestamp)
			...
			for (final int inactiveNode : inactiveVertices)
				for (int k = 0; k < model.nFactors; k++) {
					...
					S_new_den[inactiveNode][k] += gamma[c][k] * ackDiff;
				}
		...
		for (int k = 0; k < model.nFactors; k++) {
			...
			if (enableContentLikelihood)
				for (int w = 0; w < cascadeData.n_words; w++) {
					final double vv = (priorGamma_shape - 1 + Phi_new_num[w][k])
