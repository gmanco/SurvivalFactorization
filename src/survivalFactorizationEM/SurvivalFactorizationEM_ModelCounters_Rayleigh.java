...
public class SurvivalFactorizationEM_ModelCounters_Rayleigh {
	...
	public SurvivalFactorizationEM_ModelCounters_Rayleigh(int n, int k) {
		nVertices = n;
		nFactors = k;
	...
	private void resetCounters() {
		...
		resetMatrix(hat_A_c_u_k);
		Arrays.fill(A_c_k, 0);
		Arrays.fill(tilde_A_c_k, 0);
		Arrays.fill(hat_A_c_k, 0);

		resetMatrix(R_c_u_k);
		resetMatrix(tilde_R_c_u_k);
		resetMatrix(hat_R_c_u_k);

		Arrays.fill(S_c_k, 0);
		Arrays.fill(tilde_S_c_k, 0);
		Arrays.fill(hat_S_c_k, 0);

		resetMatrix(S_c_u_k);
		resetMatrix(tilde_S_c_u_k);
		resetMatrix(hat_S_c_u_k);
		...
		for (final CascadeEvent currentEvent : eventsCurrCascade) {
			...
			for (int k = 0; k < nFactors; k++) {
				if (prevEvent != null) {
					...
					hat_A_c_u_k[n][k] = prevEvent.timestamp
							* prevEvent.timestamp * model.A[prevEvent.node][k]
							+ hat_A_c_u_k[prevEvent.node][k];

					S_c_u_k[n][k] = S_c_u_k[prevEvent.node][k];
					tilde_S_c_u_k[n][k] = tilde_S_c_u_k[prevEvent.node][k];
					hat_S_c_u_k[n][k] = hat_S_c_u_k[prevEvent.node][k];
				}

				S_c_u_k[n][k] += model.S[n][k];
				tilde_S_c_u_k[n][k] += time * model.S[n][k];
				hat_S_c_u_k[n][k] += time * time * model.S[n][k];
				// FIXME forse qui c'è l'errore... n e previous node...

				tilde_A_c_k[k] += time * model.A[n][k];
				hat_A_c_k[k] += time * time * model.A[n][k];
				A_c_k[k] += model.A[n][k];

				S_c_k[k] += model.S[n][k];
				tilde_S_c_k[k] += time * model.S[n][k];
				hat_S_c_k[k] += time * time * model.S[n][k];
				...
				for (int k = 0; k < nFactors; k++)
					if (A_c_u_k[nextEvent.node][k] == 0.0) {
						if (A_c_u_k[currentEvent.node][k] != 0.0)
							...
							tilde_R_c_u_k[currentEvent.node][k] = 0;
							hat_R_c_u_k[currentEvent.node][k] = 0;
						}
					} else {
						...
						final double time = nextEvent.timestamp;
						final double frac = 1 / (time
								* A_c_u_k[nextEvent.node][k] - tilde_A_c_u_k[nextEvent.node][k]);

						hat_R_c_u_k[currentEvent.node][k] += frac
								+ hat_R_c_u_k[nextEvent.node][k];

						tilde_R_c_u_k[currentEvent.node][k] += time * frac
								+ tilde_R_c_u_k[nextEvent.node][k];
					}
